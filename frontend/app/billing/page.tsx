  ...
  const handleOpenPortal = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(
          data?.message ??
            'サブスク管理画面の起動に失敗しました。時間をおいて再度お試しください。',
        );
      ...
      if (!data.url) {
        setError('サブスク管理画面のURLが取得できませんでした。');
        setPortalLoading(false);
        return;
      }
      ...
      console.error(e);
      setError('サブスク管理画面への遷移中にエラーが発生しました。');
      setPortalLoading(false);
    }
  };
  ...
  return (
    <TenantLayout>
      <div className="p-4 space-y-6">
        ...
        {/* ★ 追加：すでに契約中の場合の注意メッセージ */}
        {isSubscriptionActive && (
          <div className="border rounded-lg p-3 bg-yellow-50 text-xs text-yellow-800">
            <p className="font-semibold mb-1">
              現在、Stripe でサブスク契約中のため、この画面から新しいプランを直接申し込むことはできません。
            </p>
            <p>
              プラン変更や解約、お支払い方法の変更は、ページ下部の「サブスクの解約・お支払い情報を変更する」ボタンから
              Stripe の管理画面（カスタマーポータル）で行ってください。
            </p>
          </div>
        )}
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <PlanCard
            ...
            // ★ active のときはボタンを無効化
            disabled={creating !== null || isSubscriptionActive}
        ...
        {/* ★ サブスク解約・支払い方法変更（Stripe ポータル） */}
        <div className="border rounded-lg p-4 bg-red-50 space-y-2">
          <h2 className="text-sm font-semibold text-red-700">
            サブスクの解約・お支払い情報の変更
          </h2>
          <p className="text-xs text-red-700">
            解約やクレジットカード情報の変更は、Stripe の管理画面から行います。
          </p>
          <button
            type="button"
            onClick={handleOpenPortal}
            disabled={portalLoading}
            className="inline-flex items-center justify-center rounded-md border border-red-400 bg-white px-4 py-2 text-xs font-semibold text-red-700 hover:bg-red-50 disabled:opacity-60"
          >
            {portalLoading
              ? 'サブスク管理画面へ遷移中...'
              : 'サブスクの解約・お支払い情報を変更する'}
