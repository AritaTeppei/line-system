...
export default function BookingsPage() {
  ...
  // 初回ロード時：auth/me + bookings + customers + cars を読む
  useEffect(() => {
    const savedToken =
  ...
  const handleCreateBooking = async () => {
    ...
    try {
      ...
      console.error(err);
      setCreateError(err.message ?? '予約作成中にエラーが発生しました');
    } finally {
      setCreating(false);
    }
  };
  ...
  // ★ 顧客を変更したときに、その顧客に属さない車が選ばれていたらリセット
  ...
  return (
    ...
    <main className="min-h-screen flex flex-col items-center p-4 gap-6 bg-gray-50">
      ...
      {me && (
        <div className="border rounded-md px-4 py-3 bg-white w-full max-w-6xl">
          ...
          <p className="text-xs text-gray-600 mt-1">
            dev@example.com（MANAGER）で、自テナントの予約を登録・確認・更新する想定です。
          </p>
        </div>
      )}
      ...
      <section className="border rounded-md px-4 py-4 bg-white w-full max-w-6xl">
        <h2 className="font-semibold mb-2">新規予約の登録（管理者用）</h2>
        <p className="text-xs text-gray-600 mb-3">
          電話や対面で受けた予約を、ここから手動で登録できます。
        </p>
        ...
        <div className="flex items-center justify-between mb-3 gap-2">
          <div>
            ...
            <p className="text-xs text-gray-600">
              車検予約・点検予約などを一覧で確認し、ステータスを更新できます。
            </p>
          </div>
          <button
